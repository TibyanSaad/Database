...
--a.
SELECT E.EMP_ID, E.FNAME ||' '|| E.LNAME AS FULL_NAME,
(SELECT COUNT(*) FROM PAYROLL P WHERE P.EMP_ID = E.EMP_ID) AS PAYROLL_COUNT
FROM EMPLOYEE E;

--b.
SELECT E.EMP_ID,E.FNAME ||' '|| E.LNAME AS FULL_NAME,
(SELECT COUNT(*) FROM LEAVE L WHERE L.EMP_ID = E.EMP_ID) AS LEAVE_COUNT
FROM EMPLOYEE E WHERE (SELECT COUNT(*) FROM LEAVE L WHERE L.EMP_ID = E.EMP_ID) 
> (SELECT COUNT(*) FROM LEAVE) / (SELECT COUNT(*) FROM EMPLOYEE);
...
WHERE E.JOB_ID = D.JOB_ID AND P.TOTAL_AMOUNT > 10000
);

--b.
SELECT E.EMP_ID, E.FNAME, E.LNAME
FROM EMPLOYEE E
WHERE NOT EXISTS ( SELECT * FROM QUALIFICATION Q WHERE Q.EMP_ID = E.EMP_ID);
